...
const CloudinaryUpload = {
    ...
    init() {
        ...
        // Support multiple possible selectors used across pages
        let dropZone = document.querySelector('.file-upload') || document.querySelector('#fileUploadArea') || document.querySelector('.file-upload-area')
        if (!dropZone) {
    ...
    async handleFiles(files) {
        ...
        const fileArray = Array.from(files).filter(file => file && file.size > 0);
    ...
    validateFile(file) {
        // Check if file is defined and has content
        if (!file || file.size === 0) {
            this.showNotification('❌ Cannot upload empty file', 'error');
            return false;
        }

        // Check if file has a valid type
        if (!file.type || !this.allowedTypes.includes(file.type)) {
            this.showNotification(`❌ Invalid file type: ${file.name || 'unknown'}`, 'error');
            return false;
        }

        // Check file size
        if (file.size > this.maxFileSize) {
            this.showNotification(`❌ ${file.name || 'File'} too large (max 5MB)`, 'error');
            return false;
        }
    ...
    showLoading() {
        const dropZone = document.querySelector('.file-upload') || document.querySelector('#fileUploadArea') || document.querySelector('.file-upload-area')
        if (!dropZone) return; // nothing to show
    ...
    let previewGrid = document.getElementById('imagePreviewGrid');
        ...
        if (!previewGrid) {
            ...
            `;
            const parentTarget = document.querySelector('.file-upload') || document.querySelector('#fileUploadArea') || document.querySelector('.file-upload-area')
            if (parentTarget && parentTarget.parentElement) parentTarget.parentElement.appendChild(previewGrid);
        }
